...
**Checkpoint snapshot**:
The versioned JSON serialization of a Run's `RunState` (messages and accounting) stored on the `runs` row. The source of truth for resume. Distinct from lifecycle signals. Reasoning is delivered as an ephemeral lifecycle signal and retained only when the Provider requires it to replay an assistant Tool call.
...
Run 的 `RunState`（messages、计数等）的版本化 JSON 序列化，存于 `runs` 行。resume 的真相来源。reasoning 作为临时生命周期信号传递；仅在 Provider 要求重放 assistant Tool call 时随该消息保留。与生命周期信号不同。
