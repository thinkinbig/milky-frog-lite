...
**Implementation (ADR-0016):** enforced through `LocalSandbox` (`harness/sandbox/`), injected via `sandbox_factory` on `AgentHarness` / `AgentSession`.
...
**实现（ADR-0016）：** 由 `LocalSandbox`（`harness/sandbox/`）执行，通过 `AgentHarness` / `AgentSession` 的 `sandbox_factory` 注入。
