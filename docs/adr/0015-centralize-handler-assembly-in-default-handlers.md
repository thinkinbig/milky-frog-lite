# Centralize handler assembly in one `default_handlers` factory
...
`handlers/assembly.py::default_handlers(settings, checkpoints, *, tool_policy,
...
already carry a `CheckpointHandler`. `default_handlers` is the one blessed path
...
- The full handler set is readable and grep-able in one place (`default_handlers`).
- Persistence-on-lifecycle becomes a **caller responsibility**. A `Harness` built
  ...
  Harness-internal; mitigated by `default_handlers` being the production path, the
...
# 将 Handler 装配集中到单一 `default_handlers` 工厂
...
已携带 `CheckpointHandler`。`default_handlers` 是保证这一点的唯一正路；测试用 `make_harness`
