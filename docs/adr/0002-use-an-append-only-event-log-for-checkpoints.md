...
An interrupted Tool remains explicitly `unknown` and requires a user decision before retrying; Milky Frog does not claim exactly-once execution. Event schemas require migrations and backward-compatibility discipline, but Python objects and provider SDK types never become the persistence format.
...
被中断的 Tool 明确保持 `unknown` 状态，重试前必须由用户决定；Milky Frog 不承诺 exactly-once 执行。事件 schema 需要迁移和向后兼容约束，但 Python 对象和模型提供方 SDK 类型不会成为持久化格式。
